...
const TokenVendor = artifacts.require('./ComposeItTokenVendor.sol');
const ComposeIt = artifacts.require('./ComposeIt.sol');

contract('ComposeIt', function (accounts) {
  const [contractOwner, alice] = accounts;
  const originalTokenPrice = 1000000000000000;

  beforeEach(async () => {
    tokenInstance = await TokenVendor.new();
    composerInstance = await ComposeIt.new(tokenInstance.address);
    await tokenInstance.setComposeItAddress(composerInstance.address);
  });
